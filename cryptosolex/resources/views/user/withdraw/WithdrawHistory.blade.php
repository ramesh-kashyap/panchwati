     ...
     <div class="contentLk">
        ...
        <h2 class="titleLk">Withdrawal History</h2>
        <div class="historyPage">
            <form id="filter-form" class="filterBl" action="{{ route('user.Withdraw-History') }}" method="GET">
                    ...
                    <div class="slideBlock branch1">
                        <div class="slideTitle" onclick="return clickRef(2, 1074);" id="el1074" data-level="1">
                            <div class="line">
                                ...
                                <span class="txt"> <span class="mobileTiTPart">Transaction ID</span>  {{ substr($value->txn_id,0,8)."*****".substr($value->transaction_id,9,8)  }}</span>
          
                                <span class="txt"> <span class="mobileTiTPart">Status</span> <span
                                class="badge badge-{{ ($value->status=='Approved')?'success':'danger' }}">{{ $value->status }}</span></span>
                            </div>
                        </div>
